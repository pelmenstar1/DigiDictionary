...
abstract class AppPreferences<TEntries : AppPreferences.Entries, TSnapshot : AppPreferences.Snapshot<TEntries>> {
    interface Entries
    data class Entry<TValue : Any, TEntries : Entries>(val valueClass: Class<TValue>, val defaultValue: TValue)
...
inline fun <reified TValue : Any, TEntries : AppPreferences.Entries> TEntries.entry(defaultValue: TValue): AppPreferences.Entry<TValue, TEntries> {
    return AppPreferences.Entry(TValue::class.java, defaultValue)
}